    ...
    if event.query.user_id == bot.uid and query.startswith("Lightning"):
        ...
        result = builder.photo(
            ...
            buttons=[
                ...
                [
                    custom.Button.inline(
                        "🛡️ I'm Here for Talking with Your Master 🛡️",
                        data="whattalk",
                    )
                ],
                [
...
async def on_plug_in_callback_query_handler(event):
    ...
    reply_pop_up_alert += "\n\n**(C)lightningsupport bot** ".format(plugin_name)
...
async def rip(event):
    if event.query.user_id == bot.uid:
        ...
        txt = "I Can't View My Masyer's Stats to you bitch!"
...
async def rip(event):
    ...
    text2 = "Ok. Please Wait until My Master Approves you. Don't Spam or Try Anything Stupid. \nThanks for Contacting me."
...
async def rip(event):
    ...
    him_id = event.query.user_id
    await event.edit("Choice Accepted ✔️")
    text3 = "Ok, Wait. You can Ask after My Master Approves you. Kindly, Wait."
...
async def inline_id_handler(event: events.InlineQuery.Event):
    if event.query.user_id != bot.uid:
        resultm = builder.article(
            title="Not Allowded",
            text=f"You Can't Use This Bot. \nDeploy вℓα¢к ℓιgнтηιηg To Get Your Own Assistant, Repo Link [Here](https://github.com/KeinShin/Black-Lightning)",
        )
...
async def inline_handler(event):
    ...
    if query == None:
        resulte = builder.article(
            title="Usage Guide.",
            description="(C) @lightningsupport",
