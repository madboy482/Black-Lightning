...
async def inline_handler(event):
    ...
    result = None
    query = event.text
    if event.query.user_id == bot.uid and query.startswith("Black Lightning"):
        rev_text = query[::-1]
        ...
        result = builder.article(
            ...
            buttons=[
                ...
                [Button.url("Join Channel ❤️", "https://t.me/blackthunderot")],
            ],
        )
        await event.answer([result])
...
async def on_plug_in_callback_query_handler(event):
    if not event.query.user_id == bot.uid:
        sedok = "Who The Fuck Are You? Get Your Own Lightning."
    ...
    reply_pop_up_alert += "\n\n**(C) @krish1303y** ".format(plugin_name)
...
async def sed(event):
    if not event.query.user_id == bot.uid:
        ...
        return
    buttons = paginate_help(0, CMD_HELP, "helpme")
    sed = f"""Black Lightning Userbot Modules Are Listed Here !\n
