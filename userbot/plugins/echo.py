...
@borg(admin_cmd(pattern="echo$"))
@borg(sudo_cmd(pattern="echo$", allow_sudo=True))
async def echo(hell):
    if hell.fwd_from:
        return
...
@borg(admin_cmd(pattern="rmecho$"))
@borg(sudo_cmd(pattern="rmecho$", allow_sudo=True))
async def echo(hell):
    if hell.fwd_from:
        return
    if hell.reply_to_msg_id is not None:
...
@borg(admin_cmd(pattern="listecho$"))
@borg(sudo_cmd(pattern="listecho$", allow_sudo=True))
async def echo(hell):
    if hell.fwd_from:
        return
    lsts = get_all_echos()
...
@borg(events.NewMessage(incoming=True))
async def samereply(hell):
    if hell.chat_id in Var.UB_BLACK_LIST_CHAT:
        return
