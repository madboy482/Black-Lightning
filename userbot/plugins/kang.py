...
async def kang(event):
    ...
    packtitle = (f"@{user.username or user.first_name}'s Lightning "
    ...
    if not muted: # Tell the user just in case, this may rarely happen
        await event.edit(
            "`Lightning  couldn't mute the Stickers bot, beware of notification spam.`")

    if new_pack:
        await event.edit("`This Sticker Is Soo Gooddd And Now Gonna Stolen\nOk Baby\n Creating a new pack...`")
        ...
        async with bot.conversation('t.me/Stickers') as conv:
            ...
            while x.text == PACK_FULL:
                ...
                number += 1
                packname = f"a{user.id}_by_{pack_username}_{number}{'_anim' if is_anim else ''}"
                packtitle = (f"@{user.username or user.first_name}'s Lightning Pack "
                            f"{number}{' animated' if is_anim else ''}")
