...
if PMPERMIT_PIC is None:
    WARN_PIC = "https://telegra.ph/file/08c516f9ff8ace1cf8865.png"
else:
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query.startswith("Thunder"):
            rev_text = query[::-1]
        ...
        if event.query.user_id == bot.uid and query == "stats":
            result = builder.article(
                title="Stats",
                text=f"**Showing Stats For {DEFAULTUSER}'s Thunder** \nNote --> Only Owner Can Check This \n(C) @ThunderOT",
                buttons=[
                    [custom.Button.inline("Show Stats ", data="terminator")],
                    [
                        Button.url(
                            "Repo 🇮🇳", "https://github.com/Anmol-dot283/Thunder"
                        )
                    ],
                    [Button.url("Join Channel ❤️", "t.me/blackthundersupport")],
                ],
            )
    ...
    async def on_plug_in_callback_query_handler(event):
        if not event.query.user_id == bot.uid:
            sedok = "Who The Fuck Are You? Get Your Own Thunder."
