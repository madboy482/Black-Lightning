...
from userbot.Plugins.sql_helper.blacklist_sql import (
    add_nibba_in_db,
    is_he_added,
    removenibba,
)
...
async def start(event):
    krishbot = await tgbot.get_me()
    bot_id = krishbot.first_name
    bot_username = krishbot.username
    replied_user = await event.client(GetFullUserRequest(event.sender_id))
    firstname = replied_user.user.first_name
    vent = event.chat_id
    starttext = f"Hello, {firstname} ! Nice To Meet You, Well I Am {bot_id}, An Thunderfull Assistant Bot. \n\nMy [➤ Master](tg://user?id={bot.uid}) \nYou Can Talk/Contact My Master Using This Bot. \n\nIf You Want Your Own Assistant You Can Deploy From Button Below. \n\nPowered By [Friday Userbot](t.me/FridayOT)"
    if event.sender_id == bot.uid:
        await tgbot.send_message(
            vent,
            message=f"Hi Master, It's Me {bot_id}, Your Thunder Assistant ! \nWhat You Wanna Do today ?",
            buttons=[
        ...
        await tgbot.send_message(
            ...
            link_preview=False,
            buttons=[
                [custom.Button.inline("Deploy your Thunder 🇮🇳", data="deploy")],
                [Button.url("Help Me ❓", "t.me/blackthundersupport")],
            ],
        )
...
async def help(event):
    ...
    if event.query.user_id is not bot.uid:
        await tgbot.send_message(
            ...
            buttons=[
                [Button.url("Deploy Tutorial 📺", "https://youtu.be/xfHcm_eQ")],
                [Button.url("Need Help ❓", "t.me/blackthundersupport")],
            ],
        )
